...
const maxInArray = (arr) => {
  let tempArr = arr.reduce((acc, item) => acc < item ? acc = item : acc, 0);
  return tempArr;
...
const hasChildrenValues = (arr, character) => {
  let tempTest = false;
  Object.values(arr).forEach(element => element.children && element.name.includes(character) ? tempTest = true : tempTest );
