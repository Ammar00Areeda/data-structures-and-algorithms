...
const addTwo = (arr) => {
  let tempArray = [];
  for (let i = 0; i < arr.length; i++) {tempArray.push(arr[i] + 2);}
  return tempArray;
};
...
const typeNum = (arr) => {
  let tempArray = arr.filter(item => typeof item === 'number' ? true : false);
  return tempArray;
};
...
const containsAnd = (arr) => {
  let tempArray = arr.filter(item => item.includes('and') ? true : false);
  return tempArray;
};
...
const oddValues = (arr) => {
  let tempArray = arr.filter(item => item%2 != 0 ? true : false);
  return tempArray;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let tempArray = arr.filter(item => !forbiddenValues.includes(item));
  return tempArray;
};
