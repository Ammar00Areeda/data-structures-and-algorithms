...
function transformToLis(obj){
  let tempArr = Object.entries(obj).map(item => `<li>${item.join(': ')}</li>`); 
  return tempArr;
};
...
const count = (target, input) => {
  let numberOfTimes = input.filter(item => item.includes(target) ? true : false).reduce((accEnd, item) => accEnd = item.reduce((acc, number) => target === number ? accEnd += 1 : accEnd, 0), 0);
...
const totalSum = (input) => {
  let totalSum = input.reduce((accEnd, item) => accEnd = item.reduce((acc, number) => accEnd += number, 0), 0)
  return totalSum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let tempArr = input.map(item => item.filter(item => typeof item === 'number' && item % 5 === 0 ? true : false).map(number => Math.pow(2, number)));
  return tempArr;
};
...
let findMaleAndFemale = (data) => {
  let namesStr = [];
  Object.values(data).forEach(item => item.gender === 'male' || item.gender === 'female' ? namesStr.push(item.name) : false);
  return namesStr.join(' and ');
};
...
let findShortest = (data) => {
  let heightTest = data.reduce((acc, item) => acc < item.height ? acc = item.height : acc, 0);
  let shortest = data.reduce((acc, item) => item.height === heightTest ? acc = item.name : acc, '');
  return shortest;
