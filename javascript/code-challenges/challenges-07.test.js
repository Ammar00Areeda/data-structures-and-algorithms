...
const sortStarWarsCharacters = (starWarsArr) => {
  let Array = starWarsArr.sort(
    (firstEl, secEl) => secEl.height - firstEl.height
  );
  return Array;
}
...
const listFoods = (recipe) => {
  let result = [];
  let tempArray = [];
  recipe.ingredients.map(item => tempArray.push(item.slice(item.indexOf(' ') + 1, item.length)));
  tempArray.map(item => result.push(item.slice(item.indexOf(' ') + 1, item.length)));
  return result;
